...
class RaydiumSwap {
  ...
  async findPoolKeys(base, quote) {
    let LP_pool = this.pool.official.find((element) => {
      if (element.baseMint === base && element.quoteMint === quote) return true;
      else return false;
    });
  ...
  async calcAmountSell(poolKeys, amount) {
    //amount in quote
    const poolInfo = await raydium.Liquidity.fetchInfo({
      connection: this.connection,
      poolKeys: poolKeys,
  ...
  async getUserTokens() {
    let userTokens = await this.connection.getTokenAccountsByOwner(
      this.publicKey,
      {
